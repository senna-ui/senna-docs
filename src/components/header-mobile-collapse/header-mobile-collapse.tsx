...
export class HeaderMobileCollapse {
  ...
  deactivate() {
    setTimeout(() => {
      this.el.classList.remove('sub-header--mobile-active');
    }, 200); // give the page a chance to load in the background so it's pretty
  }

  getTriggerEl() {
    // return (<div class="sub-header__trigger"></div>)
    const trigger = document.createElement('div');
    trigger.id = 'sub-header__trigger';
    return trigger;
  }
  ...
  handleMobileToggleClick() {
    this.el.classList.toggle('sub-header--mobile-active');
  }
