import { r as registerInstance, h, H as Host, g as getElement, c as createEvent, f as forceUpdate, a as getAssetPath } from './index-7505fb17.js';
...
const DemoForm = class {
  ...
  render() {
    return (h(Host, null, h("form", null, h("sen-fieldset", { legend: "Contact Information" }, h("sen-form-field", { label: "First name" }, h("sen-input", { type: "text" })), h("sen-form-field", { label: "Last name" }, h("sen-input", { type: "text" })), h("sen-form-field", { label: "Email" }, h("sen-input", { type: "text" })), h("sen-form-field", { label: "Gender" }, h("sen-radio", { options: genderOptions })), h("sen-form-field", { label: "Role" }, h("sen-input", { type: "text", disabled: true, value: "admin" })), h("sen-form-field", { label: "Country" }, h("sen-select", { options: countryOptions })), h("sen-form-field", null, h("sen-checkbox", { label: "Subscribe to newsletter" })), h("sen-form-field", null, h("sen-button", { color: "primary", type: "submit" }, "Submit"))))));
  }
};
...
const DemoTable = class {
  ...
  render() {
    return (h(Host, null, h("sen-data-table", { headers: tableHeaders, data: users })));
  }
};
...
const Button = class {
  ...
  render() {
    ...
    return (h(Host, null, h("button", { class: classes, type: this.type, disabled: this.disabled }, h("span", null, this.icon && h("sen-icon", { name: this.icon }), h("slot", null)))));
  }
};
...
const Checkbox = class {
  ...
  render() {
    ...
    return (h(Host, null, h("label", { id: `${this.checkboxId}-label`, onClick: this.toggle, class: labelClass }, h("input", { id: this.checkboxId, name: this.name, ref: (input) => (this.nativeInput = input), onInput: this.onInput, disabled: this.disabled, readOnly: this.readonly, checked: this.value, "aria-labelledby": `${this.checkboxId}-label`, type: "checkbox" }), h("span", { class: "checkmark" }), this.label ? this.label : h("slot", null))));
  }
...
const DataTable = class {
  ...
  render() {
    return (h(Host, null, h("sen-table", null, h("sen-table-head", null, h("sen-table-row", null, this.headers.map((header) => (h("sen-table-cell", { style: { textAlign: header.textAlign } }, header.value))))), h("sen-table-body", null, this.data.map((row) => (h("sen-table-row", null, this.headers.map((header) => (h("sen-table-cell", { align: row[header.key].textAlign || header.textAlign }, row[header.key]))))))))));
  }
};
...
const Icon = class {
  ...
  render() {
    return (h(Host, { role: "img", class: Object.assign({ "sen-icon": true }, createColorClasses(this.color)) }, h("div", { class: "icon-inner", innerHTML: this.svgContent })));
  }
...
const Radio = class {
  ...
  render() {
    ...
    return (h(Host, null, h("div", { class: elementClass }, h("span", { class: "label-text" }, this.label ? this.label : h("slot", null)), this.options.map((option) => {
      var _a, _b, _c;
      return (h("label", { class: {
          container: true,
          checked: this.value === ((_a = option.value) === null || _a === void 0 ? void 0 : _a.toString()),
        } }, h("input", { type: "radio", class: "native-radio", name: this.name, value: (_b = option.value) === null || _b === void 0 ? void 0 : _b.toString(), disabled: this.disabled, readOnly: this.readonly, onInput: (e) => this.onInput(e), checked: this.value === ((_c = option.value) === null || _c === void 0 ? void 0 : _c.toString()) }), h("span", { class: "checkmark" }), option.label));
    }))));
  }
  static get watchers() { return {
    "value": ["valueChanged"]
...
const Select = class {
  ...
  render() {
    ...
    return (h(Host, null, h("div", { class: "select-wrapper" }, h("select", { onInput: this.onInput, class: elementClass, name: this.name, disabled: this.disabled }, h("option", { disabled: this.required }, this.emptyLabel), this.options.map((option) => {
      var _a, _b, _c;
...
const SenTableCell = class {
  ...
  render() {
    return (h(Host, null, h("td", { style: { textAlign: this.align }, colSpan: this.colspan ? Number(this.colspan) : undefined, rowSpan: this.rowspan ? Number(this.rowspan) : undefined }, h("slot", null))));
  }
};
...
export { DemoForm as demo_form, DemoTable as demo_table, App as sen_app, Button as sen_button, Checkbox as sen_checkbox, Col as sen_col, DataTable as sen_data_table, Fieldset as sen_fieldset, FormField as sen_form_field, Grid as sen_grid, Icon as sen_icon, Input as sen_input, Radio as sen_radio, Row as sen_row, Select as sen_select, SenTable as sen_table, SenTableHead as sen_table_body, SenTableCell as sen_table_cell, SenTableHead$1 as sen_table_head, SenTableRow as sen_table_row };
